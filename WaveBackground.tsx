...
export const WaveBackground: React.FC<WaveBackgroundProps> = ({ dimmed = false }) => {
  ...
  useEffect(() => {
    ...
    const onScreen = (p: p5, v: any) => {
        return v.x >= -50 && v.x <= p.width + 50 && v.y >= -50 && v.y <= p.height + 50;
    }
  ...
  return (
    <div 
        ref={containerRef} 
        className={`fixed top-0 left-0 w-full h-full pointer-events-none z-0 mix-blend-screen transition-all duration-1000 ease-in-out ${dimmed ? 'opacity-30 blur-[2px]' : 'opacity-100 blur-none'}`}
    />
  );
};
